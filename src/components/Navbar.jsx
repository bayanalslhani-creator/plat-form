...
return ( <motion.nav initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3, duration: 0.6 }} dir="rtl" className={`fixed top-0 z-50 w-full transition-all duration-500 ${ scrolled || mobileMenuOpen ? "bg-black/60 backdrop-blur-md" : "bg-transparent" }`} > <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4"> {/* الشعار */} <Link to="/" className="flex items-center gap-2.5"> <img src={LOGO} alt="Plat Form Media" className="h-16 w-auto rounded-md mix-blend-screen" /> </Link>
        ...
        <div className="flex flex-col gap-5 pt-2">
          ...
          <div className="pt-2">
            <Link
              to="/join"
              onClick={() => setMobileMenuOpen(false)}
              className="block w-full rounded-lg border border-white/40 bg-white/5 py-3 text-center font-body text-xs tracking-wider text-white backdrop-blur-sm transition-colors hover:bg-white hover:text-obsidian"
            >
              سجل معنا
            </Link>
          </div>
